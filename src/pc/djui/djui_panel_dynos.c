...
void djui_panel_dynos_create(struct DjuiBase* caller) {
    ...
    if (sortedIndices == NULL) {
        // Fallback por si falla la memoria (poco probable)
        return;
    }
    ...
    // 3. Ordenar índices alfabéticamente (Bubble Sort simple)
    for (int i = 0; i < packCount - 1; i++) {
        for (int j = i + 1; j < packCount; j++) {
            ...
            // Intercambiar si nameI es mayor alfabéticamente que nameJ
...

    {
        ...
        // Iterar usando el array ordenado en lugar del contador directo
        for (int i = 0; i < packCount; i++) {
            ...
            if (dynos_pack_get_exists(packIndex)) {
                ...
                // Creamos el checkbox usando los datos del índice ordenado
                struct DjuiCheckbox* checkbox1 = djui_checkbox_create(layoutBase, pack, &tmp, djui_panel_dynos_apply);
                checkbox1->base.tag = packIndex; // Importante: guardar el ID original para que funcione 'apply'
                checkbox1->base.bTag = tmp;
                checkbox1->value = &checkbox1->base.bTag;
            }
        }
        
        // Importante: Liberar la memoria del array de ordenamiento
        free(sortedIndices); 
